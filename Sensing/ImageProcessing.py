...
COLORS["MAP"] = {
    "lower": [[124, 30, 154], [124, 30, 154], [124, 30, 154]],
    "upper": [[160, 49, 185], [160, 49, 185], [160, 49, 185]]
}
...
class ImageProcessor:
    def __init__(self, width, height):
        ...
        self.__binImage = np.zeros((height, width, 1), np.uint8)
        self.__targetImage = np.zeros((height, width, 3), np.uint8)

    def debug(self):
        numpy_horizontal_row1 = np.hstack((self.__src, cv2.cvtColor(self.__binImage, cv2.COLOR_GRAY2BGR)))
        numpy_horizontal_row2 = np.hstack((self.__targetImage, self.__src))
        combined_images = np.concatenate((numpy_horizontal_row1, numpy_horizontal_row2), axis=0)
        # cv2.imshow("debug", combined_images)
        # cv2.waitKey(1)
        return combined_images

    def getBinImage(self, color):  # 인자로 넘겨 받은 색상만 남기고 리턴
        img = self.getImage()
        ...
        self.__binImage = img_mask
        return img, img_mask

    def getBinImage_two(self, color_lst):  # 인자로 넘겨 받은 색상만 남기고 리턴
        img = self.getImage()
        img_hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    ...
    def detectTarget(self, color="RED"):
        targets = []  # 인식한 타깃들을 감지
    ...
    def meanShiftTracking_color(self, img_color, trackWindow, roi_hist, termination,
                                ):  # 추적할 대상이 정해지면 그 좌표기준으로 사각형을 그려서 추적대상을 잡는다
        need_to_update = True
        ######################## target의 업데이트 ####################
    ...
    def selectObject_many(self, mode):
        # 원하는 색깔을 주면 됨
        color = "GREEN"
    ...
    def checkDNGR_ZONE(self, color):
        ...
        if contours:
            ...
            if (area_rate) > 0.07:  # 우선 맵 밖의 영역이 50% 이상이면 위험모드
...
if __name__ == "__main__":
    ...
    while (True):
        i = imageProcessor.getBinImage(color="GREEN")
        #imageProcessor.debug()
        i = imageProcessor.selectObject_many(mode="checkCitizen")
        #imageProcessor.debug()
